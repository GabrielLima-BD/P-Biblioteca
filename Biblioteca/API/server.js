...
// Buscar livros com os GeneroID encontrados (usando operador in)
app.get('/genero', async (req, res) => {
  try {
    const NomeGenero = req.query.NomeGenero?.trim();
    if (!NomeGenero) return res.status(400).json({ message: 'NomeGenero é obrigatório' });

    const { data: generos, error: generoError } = await supabase
      .from('genero')
      .select('GeneroID, NomeGenero')
      .ilike('NomeGenero', `%${NomeGenero}%`);

    if (generoError) {
      return res.status(500).json({ message: 'Erro ao buscar gênero', error: generoError.message });
    }
    ...
    if (!generoID) {
      return res.status(500).json({ message: 'GeneroID está indefinido', genero: generos[0] });
    }
    ...
    if (livroError) {
      return res.status(500).json({ message: 'Erro ao buscar livros', error: livroError.message });
    }

    if (!livros || livros.length === 0) {
      return res.status(404).json({ message: 'Nenhum livro encontrado para o gênero' });
    }

    return res.json({ data: livros });
